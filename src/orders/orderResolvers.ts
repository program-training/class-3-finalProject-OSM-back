...
import { getAllOrdersService, getOrdersByUserIdService, updateByOrderIdService, addNewOrderService, deleteOrdersByOrderIdService, getAllOrdersServiceStatus } from "../orders/orderService";
export const orderResolvers = {
  getAllOrders: () => getAllOrdersService(),

  getOrdersByUserId: (_: unknown, { userId }: { userId: string }) => getOrdersByUserIdService(userId),

  updateOrder: (_: unknown, { orderId, updatedData }: { orderId: string; updatedData: OrderInterface }) => updateByOrderIdService(new Types.ObjectId(orderId), updatedData),
